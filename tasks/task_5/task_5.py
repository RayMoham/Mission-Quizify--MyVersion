...
class ChromaCollectionCreator:
    ...
    def create_chroma_collection(self):
        ...
        documents = self.processor.documents  # This method should exist in DocumentProcessor
        if not documents:
             st.error("No documents found! \U0001F6A8")
             return
        splitter = CharacterTextSplitter(separator='\n', chunk_size=1000, chunk_overlap=200)
        texts = splitter.split_documents(documents)
        if texts is not None:
            st.success(f"Successfully split pages to {len(texts)} documents! \U00002705")
        ...
        if self.db:
            st.success("Successfully created Chroma Collection! \U00002705")
        else:
            st.error("Failed to create Chroma Collection! \U0001F6A8")
    def query_chroma_collection(self, query) -> Document:
        ...
        if self.db:
            ...
            if docs:
                ...
                st.error("No matching documents found! \U0001F6A8")
        else:
            st.error("Chroma Collection has not been created! \U0001F6A8")
